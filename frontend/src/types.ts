    ...
    < div className = "space-y-1 overflow-y-auto flex-1" >
    {
      filteredTranscript.map((line, index) => {
        ...
        onClick = {() => onLineClick(line.start)
                    ...
                    className = {`cursor-pointer p-2 rounded-md transition-colors duration-200 flex items-start gap-3 ${isActive ? 'bg-indigo-600/50 text-white' : 'text-slate-300 hover:bg-slate-700/80'
          }`}
      >
