...
export const RefinementModal: React.FC<RefinementModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="bg-slate-800 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-700 animate-fade-in-up"
        onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside
      >
        <header className="flex justify-between items-center p-4 border-b border-slate-700">
            <div className="flex items-center gap-3">
                 <LightbulbIcon className="h-6 w-6 text-yellow-400" />
                <h2 className="font-bold text-lg text-white">Review AI Suggestion</h2>
            </div>
        ...
        <main className="p-6 space-y-6">
          <div>
            <h3 className="text-sm font-semibold text-indigo-400 mb-1">For Step:</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Current Version */}
            <div>
              <h4 className="font-semibold text-slate-300 mb-2">Current Version</h4>
            ...
            <div>
              ...
              <div className="bg-green-900/20 p-4 rounded-lg border border-green-700 h-full">
                <p className="text-sm text-green-200">{suggestion.newDescription}</p>
              </div>
            </div>
          </div>
          ...
          {suggestion.newAlternativeMethod && (
            <div>
                <h4 className="font-semibold text-green-400 mb-2">Suggested New Alternative Method</h4>
                <div className="bg-green-900/20 p-4 rounded-lg border border-green-700">
                    <h5 className="font-bold text-sm text-green-300">{suggestion.newAlternativeMethod.title}</h5>
                    <p className="text-sm text-green-200 mt-1">{suggestion.newAlternativeMethod.description}</p>
                </div>
            </div>
          )}
        ...
        <footer className="p-4 bg-slate-900/50 border-t border-slate-700 flex justify-end gap-4 rounded-b-2xl">
