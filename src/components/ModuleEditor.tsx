                        ...
                        <div key={index} className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 relative">
                            ...
                            {stepSuggestions.length > 0 && (
                                <div className="mt-4 pt-4 border-t border-amber-500/30">
                                    <h4 className="text-sm font-semibold text-amber-400 mb-2 flex items-center gap-2"><SparklesIcon className="h-4 w-4" />Pending AI Suggestions</h4>
                                    <div className="space-y-3">
                                        {stepSuggestions.map(suggestion => (
                                            <div key={suggestion.id} className="bg-amber-900/40 p-3 rounded-lg">
                                                <p className="text-sm text-slate-300 italic">"{suggestion.text}"</p>
                                                <div className="flex items-center justify-end gap-3 mt-3">
                                                    <button onClick={() => onRejectSuggestion(suggestion.id)} className="text-xs text-slate-400 hover:text-white font-semibold py-1 px-3 rounded-full hover:bg-slate-600 transition-colors">
                                                        Reject
                                                    </button>
